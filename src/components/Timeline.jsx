    ...
    <div className="c-space section-spacing" ref={containerRef}>
      ...
      <div className="relative pb-20">
        {data.map((item, index) => {
          ...
          // Track dot activation based on scroll
          useEffect(() => {
            ...
            const unsubscribe = lineHeight.onChange((value) => {
              ...
              const dotCenter =
                dot.getBoundingClientRect().top + window.scrollY - containerTop + dot.offsetHeight / 2;

              setIsActive(value >= dotCenter - navbarHeight);
            });
            return () => unsubscribe();
          }, [lineHeight, index]);
            ...
            <div key={index} className="flex justify-start pt-10 md:pt-40 md:gap-10">
              <div className="flex-col flex z-40 items-center self-start max-w-xs md:flex-row md:sticky md:top-40 lg:max-w-sm md:w-full mt-10">
                ...
                <div className="absolute flex items-center justify-center w-10 h-10 -left-[15px] bg-midnight rounded-full">
                  <motion.div
                    ...
                    style={{
                      boxShadow: isActive
                        ? `
                          0 0 15px 6px rgba(167,139,250,0.8),
                          0 0 30px 12px rgba(167,139,250,0.6),
                          0 0 45px 18px rgba(167,139,250,0.4)
                        `
                        : "none",
                ...
                <div className="flex-col hidden gap-2 text-xl font-bold md:flex md:pl-20 md:text-4xl text-neutral-300">
                  <h3 className="text-sm text-neutral-400">{item.date}</h3>
                  <h3 className="text-3xl text-neutral-400">{item.title}</h3>
                  <h3 className="text-3xl text-neutral-500">{item.company || item.job}</h3>
                </div>
              </div>
